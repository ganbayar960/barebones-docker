...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "barebones", autostart: true do |solo|
    solo.vm.provider "docker" do |docker|
      docker.name = PROJECT_ENV
      docker.build_dir = "."
      docker.link("mysql-" + PROJECT_ENV+ ":mysql")
      docker.env = {
        VIRTUAL_HOST: PROJECT_ENV + ".*, barebones.*",
        FARCRY_DSN: "barebones",
        FARCRY_DBTYPE: "mysql",
        FARCRY_DSN_CLASS: "org.gjt.mm.mysql.Driver",
        FARCRY_DSN_CONNECTIONSTRING: "jdbc:mysql://mysql:3306/barebones?useUnicode=true&characterEncoding=UTF-8&allowMultiQueries=true&useLegacyDatetimeCode=true",
  ...
  ##################################################
  config.vm.define "barebonesh2", autostart: false do |solo|
    solo.vm.provider "docker" do |docker|
      ...
      docker.env = {
        ...
        FARCRY_DSN_CONNECTIONSTRING: "jdbc:h2:barebones/chelsea;MODE=MySQL",
        FARCRY_DSN_USERNAME: "vagrant",
        FARCRY_DSN_PASSWORD: "vagrant"
      }
      # local development code, lucee config & logs
